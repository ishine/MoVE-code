...
class KimiAudio(object):
    ...
    def _generate_loop(
        ...
        audio_repetition_penalty: float = 1.12,
        audio_repetition_window_size: int = 5,
        text_top_k: int = 5,
        ...
        # === CONFIGURATION ===
        ENABLE_DEBUG_PRINT = enable_debug_print
        TARGET_EOD_INDEX = 1 
        # =====================
        ...
        for i in tqdm.tqdm(
            ...
            if ENABLE_DEBUG_PRINT:
                target_token_id = self.eod_ids[TARGET_EOD_INDEX]
                current_logits = audio_logits[0, -1, :] 
                probs = torch.nn.functional.softmax(current_logits, dim=-1)
                
                # Selected token info
                sel_id = next_audio_token.item()
                sel_prob = probs[sel_id].item()
                sel_logit = current_logits[sel_id].item()
                ...
                print(f"[Step {i}] Selected: {sel_id} (P={sel_prob:.4f} L={sel_logit:.2f}) | "
                      f"Max: {max_idx.item()} (P={max_prob.item():.4f} L={max_logit:.2f}) | "
    ...
    def generate(
        ...
        ## TODO: need a check function to validate the input history format.
        ## e.g. for ASR tasks it must be: text-instruction/audio-instruction + audio-content;
        ## content and instruction are not interchangeable.
        ## assistant turns must be preceded by user turns, etc. — worth validating.
        ...
        #else:
        if max_new_tokens == -1:
            max_new_tokens = audio_input_ids.shape[1] + 25
        ...
        return generated_wav_tokens, generated_text
