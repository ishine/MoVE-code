...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        import os
        rk = os.environ.get("RANK") or os.environ.get("LOCAL_RANK") or os.environ.get("SLURM_PROCID") or "0"


        embed_dim = config.d_model
        #print(f"[DBG][Rank{rk}][whisper] grad_enabled={torch.is_grad_enabled()} training={self.training} config.d_model = {config.d_model} embed_dim = {embed_dim}")
        ...
        #print(f"[DBG][rank{rk}] self.embed_positions.weight.shape = {self.embed_positions.weight.shape} = nn.Embedding({self.max_source_positions}, {embed_dim})")
        #print(f"[DBG][rank{rk}][whisper] grad_enabled={torch.is_grad_enabled()} training={self.training} self.embed_positions.weight.shape = {self.embed_positions.weight.shape} ")
    ...
    def forward(
        ...
        #import os
        #rk = os.environ.get("RANK") or os.environ.get("LOCAL_RANK") or os.environ.get("SLURM_PROCID") or "0"
        #print(f"[DBG][whisper] grad_enabled={torch.is_grad_enabled()} training={self.training} [rank{rk}] embed_pos.shape = {embed_pos.shape} max_source_positions={self.max_source_positions}")
        #w = getattr(self.embed_positions, "weight", None)
        #print(f"[DBG][whisper] embed_positions.weight={None if w is None else tuple(w.shape)}")

        #print(f"[rank{rk}] ==== DEBUG in modeling_whisper.py:1041 ====")
        #print(f"[rank{rk}]embed_positions.weight.shape = {self.embed_positions.weight.shape}")
        #print(f"[rank{rk}] self.max_source_positions = {self.max_source_positions}")
        #print(f"[rank{rk}] embed_pos.shape = {embed_pos.shape}")
        #print(f"[rank{rk}] ===========================================")
