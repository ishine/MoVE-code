...
class LazySupervisedDataset(Dataset):
    ...
    def tokenize_message(
        ...
        # 處理 audio-text 混合輸入的情況
        elif message["message_type"] == "audio-text": 
            _, text = message["content"] 
            text_tokens = self._tokenize_text(text) 
            speech_tokens = message["audio_tokens"]

            kimia_content_msg.audio_extend([self.extra_tokens.kimia_text_blank] * AUDIO_DELAY_TOKENS, audio_token_loss_mask=False) 
            kimia_content_msg.audio_extend(speech_tokens, is_continuous=False, audio_token_loss_mask=has_loss) 
            kimia_content_msg.text_extend(text_tokens, text_token_loss_mask=has_loss)
            kimia_content_msg.text_append(self.extra_tokens.kimia_text_eos, text_token_loss_mask=has_loss) # eos for text stream
            text_pad_tokens = (AUDIO_DELAY_TOKENS + len(speech_tokens) - len(text_tokens) - 1) * [self.extra_tokens.kimia_text_blank] 
            kimia_content_msg.text_extend(text_pad_tokens, text_token_loss_mask=False) 
        ...
        if has_msg_end_token:
            kimia_content_msg.audio_append(self.extra_tokens.msg_end, audio_token_loss_mask=has_loss)
            kimia_content_msg.text_append(self.extra_tokens.kimia_text_blank)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        output_type = "text" if task_type == "s-t" else "both"
